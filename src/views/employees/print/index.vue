...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      const res = await getUserDetail(this.userId) // 获取个人基本信息(顶部)
      const msg = await getPersonalDetail(this.userId) // 获取个人基本信息(底部)
      this.formData = { ...msg, ...res }
    },
    async getJobDetail() {
      const res = await getUserDetail(this.userId)
      const msg = await getJobDetail(this.userId) // 获取个人基本信息
